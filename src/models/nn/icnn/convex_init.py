...
class ConvexBiasCorrectionInitialiser:
    ...
    def __init__(self, positivity, gain: float = 1.0):
    ...
    def __call__(self, weight: torch.Tensor, bias: torch.Tensor):
        ...
        offset, dist = weight_mean_sq**0.5, (3.0 * weight_var) ** 0.5
    ...
    def compute_parameters(
        ...
        # pytorch default init uses uniform distribution
        w_tmp = self.positivity(
            (2.0 * torch.rand(10_000) - 1.0) * (3.0 * weight_var) ** 0.5
        )
        shift = fan_in * w_tmp.mean() / (2 * torch.pi) ** 0.5
        return (0.0, weight_var), (-shift, 0.0)
...
class ConvexInitialiser:
    ...
    def compute_parameters(
        ...
        shift = (
            fan_in * (target_mean_sq * self.target_var / (2 * math.pi)) ** 0.5
        )
        bias_var = 0.0
        if self.bias_noise > 0.0:
            target_variance *= 1 - self.bias_noise
            bias_var = (
                self.bias_noise * (1.0 - self.target_corr) * self.target_var
            )

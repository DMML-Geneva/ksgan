...
class ConvexLinear(nn.Linear):
    ...
    def __init__(self, *args, positivity: Positivity = None, **kwargs):
        if positivity is None:
            raise TypeError(
                "positivity must be given as kwarg for convex layer"
            )
    ...
    def forward(self, x):
        ...
        if self.training:
            ...
            if self.training:
                ...
                self.running_mean = (
                    1 - self.momentum
                ) * self.running_mean + self.momentum * torch.mean(mean)
                self.running_var = (
                    1 - self.momentum
                ) * self.running_var + self.momentum * torch.mean(var)
        else:
...
class LinearSkip(nn.Module):
    ...
    def __init__(
        self, in_features: int, out_features: int, residual: nn.Module
    ):
        super().__init__()
...
class Conv2dSkip(nn.Module):
    ...
    def __init__(
        self, in_channels: int, out_channels: int, residual: nn.Module
    ):
        super().__init__()

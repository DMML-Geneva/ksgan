...
def main():
    ...
    for dataset in datasets:
        for model in group:
            for sb in cfg["simulation-budgets"]:
                for seed in cfg["model_seeds"]:
                    ...
                    for metric_file in (
                        "metrics-summary-rsample.json",
                        "metrics-summary-log_prob.json",
                        "metrics-summary-critic.json",
                    ):
                        if (
                            os.stat(
    ...
    pd.DataFrame.from_records(records).to_csv(
        os.path.join(args.out_path, "metrics.csv")
    )

...
if __name__ == "__main__":
    ...
    if model_has_log_prob(args.model):
        evaluate_log_prob(
        ...
        open(
            os.path.join(
                args.metrics_summary_path, "metrics-summary-log_prob.json"
            ),
            "w",
        ).close()
    ...
    if model_has_critic(args.model):
        evaluate_critic(
            model_name=args.model,
            simulator=args.simulator,
            model_config=deepcopy(model_cfg),
        ...
        open(
            os.path.join(
                args.metrics_summary_path, "metrics-summary-critic.json"
            ),
            "w",
        ).close()
